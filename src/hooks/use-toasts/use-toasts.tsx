import { toastMessagesType, toastType, toastProviderType } from '_/utils/types';
...
export const ToastProvider: React.FC<providerProps> = ({
  ...
  const [state, setState] =
    React.useState<toastMessagesType>(TOAST_DEFAULT_DATA);
    ...
    <Context.Provider value={{ ...state, ...methods }}>
      ...
      <ToastContainer>
        {React.Children.toArray(
          state.toasts.map((item): JSX.Element => {
            return (
              <ToastMessage variant={item.variant}>{item.message}</ToastMessage>
            );
          }),
        )}
      </ToastContainer>
...
export const useNotification = () => {
  const context = React.useContext(Context) as toastProviderType;
